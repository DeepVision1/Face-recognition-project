...
def setup_database():
    cursor.execute("""
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='faces' AND xtype='U')
    CREATE TABLE faces (
        id INT IDENTITY(1,1) PRIMARY KEY,
        name NVARCHAR(255),
        Face_embedding VARBINARY(MAX),
        Voice_embedding VARBINARY(MAX)
    )
    """)
    
    cursor.execute("""
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='recognition_log' AND xtype='U')
    CREATE TABLE recognition_log (
        id INT IDENTITY(1,1) PRIMARY KEY,
        person_id INT,
        timestamp DATETIME DEFAULT GETDATE(),
        similarity FLOAT,
        verification_type NVARCHAR(50),
...
def get_logs():
    ...
    cursor.execute("""
        SELECT TOP 10 
            ...
            r.similarity, 
            r.verification_type
        FROM recognition_log AS r
        JOIN faces AS f ON r.person_id = f.id
